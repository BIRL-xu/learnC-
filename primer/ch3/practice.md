...
// 从括号往外读，Parray是一个指针，它指向int数组，大小是10。 因此，Parray是指向一个含有10个整数的数组的指针。
int (*Parray)[10] = &arr; 
...
#### C风格字符串，即字符数组
````c++
// C风格字符串函数，参数必须是以空字符结尾的字符数组。
strlen(p); // 返回p的长度，不包括空字符。
strcmp(p1, p2) // 比较p1和p2的相等性。如果p1==p2,返回０；如果p1>p2，返回一个正值;如果p1<p2,返回一个负值。
strcat(p1, p2) // 将p2附加到p1之后，返回p1。
strcpy(p1, p2) // 将p2拷贝给p1，返回p1。
````
----
#### Q3.37: 下面的程序是何含义，程序的输出结果是什么？
````c++
const char ca[] = {'h', 'e', 'l', 'l', 'o'}; // 常量，不能递增
const char *cp = ca; // 指向字符常量的普通指针，可以递增。
while(*cp)
{
    cout << *cp << endl;
    ++cp;
...
````
#### A: 以指针访问每一个字符常量。输出hello.
#### Q3.38: 在本节中我们提到，将两个指针相加不但是非法的，而且也没什么意义。请问为什么两个指针相加没什么意义？
#### A: 指针是用来代表内存地址的。指针的数值是该地址相对于最低位地址也就是0位地址的偏移量，也可称之为坐标。坐标相加得到的新值是没什么意义的，坐标相减则是距离，坐标加距离则是新坐标，后两者是有意义的
#### Q3.39: 编写一段程序，比较两个string对象。再编写一段程序，比较两个C风格字符串的内容。
#### A: 见3-39.cpp。
#### Q3.40: 编写一段程序，定义两个字符数组并用字符串字面值初始化它们；接着再定义一个字符数组存放前两个数组连接后的结果。使用strcpy和strcat把前两个数组的内容拷贝到第三个数组中。
#### A: 见3-40.cpp。
#### string的函数`const *str = s.c_str()`返回的指向一个以空字符结束的字符数组的指针，如果后续s被修改，会导致str的值也被修改！！！！因此，最好拷贝一份字符数组
* const只能保证不能经过str修改s的值，但是不能保证s不被其它方法修改，如直接给s赋值，这样str指向的字符也被改变了。
##### 使用数组初始化vector对象
````c++
int arr[] = {0, 1, 2, 3, 4, 5};
vector<int> ivec(begin(arr), end(arr)); // 指明拷贝区域的首地址和尾元素的下一个地址。
````
#### Q3.41: 编写一段程序，用整型数组初始化一个vector对象。
#### A: 见3-41.cpp。
#### Q3.42:　编写一段程序，将含有整数元素的vector对象拷贝给一个整型数组.
#### A: 只能每个元素单独拷贝， 见3-42.cpp。
#### 多维数组：　一个维度表示数组本身大小，另外一个维度表示其元素(也是数组)的大小。
````c++
int ia[3][4]; // 大小为３的数组，每个元素是含有4个整数的数组。
````
#### 多维数组初始化：
````c++
int ia[3][4] = {
...
for(size_t i = 0; i != rowCnt; ++i)
    for(size_t j = 0; j != colCnt); ++j)
        ia[i][j] = i * colCnt + j;
// 范围for
// 除了最内侧的循环外，其他所有循环的控制变量都必须是引用类型，因为
// 不用引用的话，编译器初始化row时会自动将它转换成指向该数组首元素的指针，
// 这样得到的row就是int*类型，无法进行内存循环。
size_t cnt = 0;
for(auto &row : ia)
    for(auto col : row)
        cout << col << endl;
...
// 使用二层循环和指针
int a[3][4];
for(auto p = ia; p != ia +3; ++p)   // p是指向３个整数数组，*p为其中一个数组。
    for(auto q = *p; q != (*p + 4); ++q) // q指向是指向4个元素数组的指针.
        cout << *q << endl;
// 迭代器
